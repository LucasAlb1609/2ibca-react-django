...
function PaginaVerUsuario() {
  ...
  const { userId } = useParams(); // Hook para pegar o ID da URL (ex: /admin/ver-usuario/5)
    ...
    <div className="bg-gray-100 p-4 sm:p-8 font-sans not-italic">
      <div className="max-w-5xl mx-auto bg-white rounded-lg shadow-xl overflow-hidden">
        <div className="bg-blue-600 text-white p-4 flex justify-between items-center">
          <h3 className="text-2xl font-semibold">Perfil de {profile.nome_completo}</h3>
          <Link to={`/admin/editar-usuario/${profile.id}`} className="bg-yellow-400 text-black font-bold text-sm py-2 px-4 rounded-md hover:bg-yellow-500 transition-colors">
            Editar Usuário
          </Link>
        </div>
        <div className="p-6">
          ...
          <div className="text-center mt-8">
            <Link to="/admin/todos-usuarios" className="text-blue-600 hover:underline font-semibold">Voltar à Lista de Usuários</Link>
          </div>
        </div>
      </div>
    </div>

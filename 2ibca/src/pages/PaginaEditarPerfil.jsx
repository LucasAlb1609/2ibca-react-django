...
function PaginaEditarPerfil() {
  ...
  // Efeito para buscar os dados atuais do perfil
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        ...
        // Inicializa o estado com os dados recebidos
        setFormData({
            ...data,
            tem_filhos: data.filhos.length > 0 ? 'True' : 'False',
  ...
  const handleSubmit = async (e) => {
    ...
    const editableFields = ['nome_completo', 'email', 'foto_perfil', 'data_nascimento', 'nome_pai', 'nome_mae', 'cpf', 'rg', 'naturalidade', 'estado_civil', 'nome_conjuge', 'data_casamento', 'telefone', 'endereco', 'bairro', 'cidade', 'cep', 'profissao', 'nivel_escolar', 'data_conversao', 'batizado_aguas', 'data_batismo', 'local_batismo', 'outra_igreja_batismo', 'recebido_por_aclamacao', 'membro_congregacao', 'qual_congregacao', 'frequenta_escola_biblica', 'qual_classe_escola_biblica', 'deseja_exercer_funcao', 'qual_funcao_deseja', 'tem_alergia_medicacao', 'alergias_texto'];
  ...
  if (loading || !formData) return <div className="text-center p-10 font-semibold italic">Carregando formulário de edição...</div>;
        ...
        <div className="max-w-4xl mx-auto">
          <div className="bg-white rounded-lg shadow-xl overflow-hidden">
            <div className="bg-blue-600 text-white p-4">
              <h3 className="text-2xl font-semibold">Editar Meu Perfil</h3>
            </div>
            <div className="p-6 md:p-8">
              <form onSubmit={handleSubmit} noValidate encType="multipart/form-data" className="space-y-8">
                ...
                <div className="flex justify-between items-center mt-8">
                  <Link to="/perfil" className="text-gray-600 hover:text-blue-600 transition-colors">Cancelar</Link>
                  <button type="submit" disabled={loading} className="bg-green-600 text-white font-bold py-3 px-8 rounded-lg hover:bg-green-700 transition-colors disabled:bg-gray-400 disabled:cursor-not-allowed">
